...
class AlamofireAdapter {
    ...
    func post(to url: URL, with data: Data?, completion: @escaping (Result<Data, HttpError>) -> Void) {
        session.request(url, method: .post, parameters: data?.toJson(), encoding: JSONEncoding.default).responseData { dataResponse in
            guard (dataResponse.response?.statusCode) != nil else { return completion(.failure(.noConnectivityError))}
...
class AlamofireAdapterTests: XCTestCase {
    ...
    func test_post_should_complete_with_error_all_ivalid_cases() {
        ...
        expectResult(.failure(.noConnectivityError), when: (data: nil, response: nil, nil))
...
extension AlamofireAdapterTests {
    ...
    func expectResult(_ expectedResult: Result<Data, HttpError>, when stub: (data: Data?, response: HTTPURLResponse?, error: Error?), file: StaticString = #filePath, line: UInt = #line) {
        let sut = makeSut()
        UrlProtocolStub.silumate(data: stub.data, response: stub.response, error: stub.error)
        let exp = expectation(description: "waiting")
...
class UrlProtocolStub: URLProtocol {
    ...
    static func silumate(data: Data?, response: HTTPURLResponse?, error: Error?) {
